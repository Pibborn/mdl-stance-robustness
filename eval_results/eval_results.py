...
if __name__ == '__main__':
    ...
    for eval_dataset in eval_datasets.split(","):
        for s in sets.split(","):
            ...
            eval_file_name = "eval_results_" + eval_dataset + "_" + s + ".json"

            with open(path+"/"+result_file, "r") as f_in:
                results = json.load(f_in)
            
            results = add_seed_avg_key(results, set_key=s)

            with open(path+"/"+eval_file_name, "w") as f_out:
