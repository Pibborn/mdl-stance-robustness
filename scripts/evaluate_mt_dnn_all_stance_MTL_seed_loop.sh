#!/bin/bash
if [[ $# -ne 6 ]]; then
  echo "bash evaluate_mt_dnn_all_stance_MTL_seed_loop.sh <gpu> <model_name> <basedir> <timestamp> <train_data_ratio> <lambda>"
...
for seed in "${seeds[@]}" ; do
    if [[ $train_data_ratio -eq 100 ]]; then
        model_dir="${basedir}/debias_checkpoints/${prefix}_seed${seed}_ep${epochs}_${model}_answer_opt${answer_opt}_lambda${lambda}_${tstr}"
    else
        model_dir="${basedir}/debias_checkpoints/${prefix}_seed${seed}_ep${epochs}_${model}_answer_opt${answer_opt}_trainratio${train_data_ratio}_lambda${lambda}_${tstr}"
    fi
    echo $model_dir
    BERT_PATH="${model_dir}/model.pt"
    cp "../mt_dnn_models/vocab.txt" $model_dir
    log_file="${model_dir}/log.log"
    python -W ignore ../predict.py --train_data_ratio ${train_data_ratio} --dump_to_checkpoints ${dump_to_checkpoints} --stress_tests ${stress_tests} --max_seq_len ${max_seq_len} --seed ${seed} --epochs ${epochs} --data_dir ${DATA_DIR} --init_checkpoint ${BERT_PATH} --batch_size ${BATCH_SIZE} --output_dir ${model_dir} --log_file ${log_file} --answer_opt ${answer_opt} --optimizer ${optim} --train_datasets ${test_datasets} --test_datasets ${test_datasets} --grad_clipping ${grad_clipping} --global_grad_clipping ${global_grad_clipping} --learning_rate ${lr} --dump_representations
done
